 ...
 */
export class MediaNodeRunnerProxy implements NodeRunner {
    ...
    private async getRunner(): Promise<NodeRunner> {
        ...
        if (this.environment === 'browser') {
            ...
            this.runner = new MediaNodeRunnerBrowser();
        } else {
            // Server implementation - dynamically import to avoid bundling Node.js modules in browser
            const { MediaNodeRunnerServer } = await import('./MediaNodeRunnerServer');
            this.runner = new MediaNodeRunnerServer();
        }
